...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/17 16:10:10 by dmelnyk           #+#    #+#             */
/*   Updated: 2025/04/17 16:10:14 by dmelnyk          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void think_routine(t_philo *philo, bool silent) {
    ...
    time_to_think = (long)(philo->info->die - (get_current_time() - philo->times.last_meal) -
                           philo->info->eat) /
                    2;
...
bool launch(t_table *table) {
    ...
    if (table->info.philosophers == 1) {
        if (pthread_create(&table->philos[0].thread, NULL, single_phil, &table->philos[0]) != 0)
    ...
    while (++i < table->info.philosophers) {
        if (pthread_create(&table->philos[i].thread, NULL, start_philosopher, &table->philos[i]) !=
            0)
            return (false);
    }
